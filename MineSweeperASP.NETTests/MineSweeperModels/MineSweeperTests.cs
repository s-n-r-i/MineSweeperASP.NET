...
#pragma warning restore CS8618 // null 非許容のフィールドには、コンストラクターの終了時に null 以外の値が入っていなければなりません。Null 許容として宣言することをご検討ください。
    ...
    public void RestoreTest()
    {
        ...
        Assert.AreEqual(ColumnCount, mineSweeper.ColumnCount);
        Assert.AreEqual(BombCount, mineSweeper.BombCount);
        Assert.AreEqual(RowCount * ColumnCount - BombCount - 1, mineSweeper.RemainingCellCount);
        Assert.AreEqual(29, remainingCells.Count());
        Assert.IsFalse(remainingCells.Any(x => x.Index == 0));
    }
}
